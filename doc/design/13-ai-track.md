...
パネル ID は `storyboardBoard`。他のパネルと同様にドック位置とタブ構成を変更でき、
既定ではタイムラインと同じ下エリアのタブグループに入る ([1.7](01-architecture.md#17-ui-パネル構成-ドッキング--タブ))。
...
- パネル ID は `aiTasks`。タブ可能パネルの 1 つで、既定ではインスペクタと同じ右エリアの
  タブグループに入る ([1.7](01-architecture.md#17-ui-パネル構成-ドッキング--タブ))
